...
function playlist_page_href(array $params, int $page): string {
  $params['page'] = $page;
...
<div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
  <div>
    <h1 class="h4 m-0"><i class="bi bi-collection-play me-2" aria-hidden="true"></i><?= e((string)($playlist['name'] ?? 'Playlist')) ?></h1>
    <div class="text-muted small"><?= (int)($playlist['song_count'] ?? 0) ?> songs</div>
  </div>
  <a class="btn btn-outline-secondary btn-sm" href="<?= e(APP_BASE) ?>/?r=/playlists"><i class="bi bi-arrow-left me-1" aria-hidden="true"></i>Back</a>
</div>

<div class="card shadow-sm mb-3">
  <div class="card-body">
    <form method="get" action="<?= e(APP_BASE) ?>/" id="songsSearchForm" class="row g-2 align-items-end" data-page-route="/playlist" data-api-route="/api/playlist-songs">
      <input type="hidden" name="r" value="/playlist">
      <input type="hidden" name="id" value="<?= (int)($playlist['id'] ?? 0) ?>">
    ...
    <nav aria-label="Playlist pages">
      <ul class="pagination mb-0">
        <li class="page-item <?= $pager->hasPrev() ? '' : 'disabled' ?>">
          <a class="page-link" href="<?= playlist_page_href($params, $pager->prevPage()) ?>" data-page="<?= (int)$pager->prevPage() ?>">Prev</a>
        </li>
        <?php foreach ($pager->window(2) as $p): ?>
          <li class="page-item <?= $p === $pager->page ? 'active' : '' ?>">
            <a class="page-link" href="<?= playlist_page_href($params, (int)$p) ?>" data-page="<?= (int)$p ?>"><?= (int)$p ?></a>
          </li>
        <?php endforeach; ?>
        <li class="page-item <?= $pager->hasNext() ? '' : 'disabled' ?>">
          <a class="page-link" href="<?= playlist_page_href($params, $pager->nextPage()) ?>" data-page="<?= (int)$pager->nextPage() ?>">Next</a>
        </li>
      </ul>
    </nav>
  <?php endif; ?>
