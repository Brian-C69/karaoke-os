...
<div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
  ...
  <div class="d-flex align-items-center gap-2">
    ...
    <a class="btn btn-outline-secondary btn-sm" href="<?= e(APP_BASE) ?>/?r=/songs&sort=plays"><i class="bi bi-music-note-list me-1" aria-hidden="true"></i>All songs</a>
  </div>
</div>

<?php if (!$rows): ?>
  <div class="alert alert-info">No songs yet.</div>
<?php else: ?>
  <?php if ($view === 'list'): ?>
    <div class="list-group shadow-sm">
      <?php $rank = 1; foreach ($rows as $r): ?>
        <a class="list-group-item list-group-item-action d-flex align-items-center gap-3" href="<?= e(APP_BASE) ?>/?r=/song&id=<?= (int)$r['id'] ?>">
          ...
          <div class="text-muted small text-nowrap"><?= (int)$r['play_count'] ?> plays</div>
        </a>
      <?php $rank++; endforeach; ?>
    </div>
  <?php else: ?>
        ...
        <div class="col-6 col-md-4 col-lg-3">
          <a class="card song-card h-100 shadow-sm text-decoration-none" href="<?= e(APP_BASE) ?>/?r=/song&id=<?= (int)$r['id'] ?>">
            ...
            <div class="card-body">
              ...
              <div class="text-muted small text-truncate"><?= e((string)$r['artist']) ?></div>
              <div class="text-muted small"><?= (int)$r['play_count'] ?> plays</div>
            </div>
          </a>
        </div>
      <?php $i++; endforeach; ?>
